...
final class AacpV25Probe {
    ...
    AacpV25Probe(BluetoothDevice device, LogSink log) {
        this.device = device;
        this.log = log;
    }

    void run() {
        log("=== AirPods AACP v25 probe started ===");
        ...
        log("--- v25 ATT 0x002A reversible one-ULP canary write test ---");
        log("v24 proved that ATT handle 0x002A accepts an exact-current Write Request, but a pure no-op produced no direct AACP response and no read-back change.");
        log("v25 therefore tests semantic writability with the smallest practical reversible canary: change only the final float32-le value by one ULP, read back, then restore the original bytes.");
        log("v25 uses the same winning v20/v22 capture path, reads ATT 0x002A before and after CCCD 0x0022, and requires the value to be stable before writing anything.");
        log("v25 sends no AACP 0x0052, 0x0053, 0x0054, or 0x0055 candidates. The only non-current write is the guarded one-ULP ATT 0x002A canary, followed immediately by an original-value restore.");
        log("The canary guard requires the observed 104-byte 0x002A shape, selector byte0=0x02, length word [2..3]=96, and a finite final float in the safe [0,1] range.");
        ...
        for (int i = 1; i <= attempts; i++) {
            ...
            if (completed) break;
            if (i < attempts) {
                log("v25: attempt " + i + " did not reach the guarded reversible 0x002A canary path. Sleeping briefly, then retrying the same safe path once.");
                sleep(1300);
            }
        }
        if (!completed) {
            log("v25 final result: no stable guarded ATT 0x002A value was available for a reversible one-ULP canary. No canary write was sent.");
        }
        log("=== Probe finished ===");
    }
    ...
    private boolean runCaptureAndWriteAttempt(int attempt, int totalAttempts) {
        ...
        String prefix = "v25 attempt " + attempt + "/" + totalAttempts;
        try {
            ...
            if (context53 != null) {
                ...
                log(prefix + " note: no current-session AACP 0x0053 context was captured, but v25 can still test ATT 0x002A only if the ATT value is stable.");
            }
            ...
            byte[] canary2a = deriveOneUlpCanary2a(postCccd2a);
            if (canary2a == null || ByteUtil.equalsBytes(canary2a, postCccd2a)) {
                log(prefix + " abort: failed to derive a distinct one-ULP canary from the current ATT 0x002A value. No canary write was sent.");
                return false;
            }
            ...
            log(prefix + " one-ULP canary changes only final float32-le at offset " + finalFloatOffset
            ...
            log(prefix + " one-ULP canary payload bytes=" + ByteUtil.hex(canary2a));

            boolean canaryWriteAttempted = false;
            boolean canaryWriteOk = false;
            boolean restoreWriteOk = false;
            try {
                log(prefix + ": sending one ATT Write Request to handle 0x002A with the one-ULP canary value.");
                canaryWriteAttempted = true;
                canaryWriteOk = attWriteRequest(att, HANDLE_HEARING_CONFIG, canary2a, prefix + " one-ULP canary write 0x002A");
                log(prefix + ": one-ULP canary ATT 0x002A write result: " + canaryWriteOk);

                drainAtt(att, prefix + " ATT after one-ULP canary 0x002A write", 40, 2200);
                drainAacp(aacp, prefix + " AACP direct drain after one-ULP canary ATT 0x002A write", 140, 10000, 260, postCanary);
            ...
            log("  one-ULP canary ATT 0x002A write attempted: " + canaryWriteAttempted);
            log("  one-ULP canary ATT 0x002A write request accepted: " + canaryWriteOk);
            ...
            if (canaryWriteOk && afterCanary2a != null && ByteUtil.equalsBytes(canary2a, afterCanary2a)) {
                log(prefix + " interpretation: ATT 0x002A is semantically writable at byte level: the one-ULP canary read back exactly. Restore status should be checked before any larger test.");
            } else if (canaryWriteOk && afterCanary2a != null && ByteUtil.equalsBytes(postCccd2a, afterCanary2a)) {
                log(prefix + " interpretation: ATT accepted the canary write, but 0x002A read-back stayed original. That suggests the app ignored/quantized the one-ULP change or the write path needs a commit/refresh trigger.");
                ...
                log(prefix + " interpretation: ATT 0x002A rejected the one-ULP canary write, despite accepting exact-current no-op in v24.");
            }
            return true;
        } catch (Exception e) {
    ...
    private boolean validate2aForCanary(byte[] value) {
        ...
        log("v25 selected ATT 0x002A canary validation: len=" + value.length + ", nonZero=" + ByteUtil.nonZero(value));
        decode2a("v25 selected ATT 0x002A canary source decode", value);
        if (value.length != 104) {
            log("v25 0x002A canary guard: refusing because the observed safe shape is exactly 104 bytes, but current length is " + value.length + ".");
            return false;
        }
        if (ByteUtil.u8(value[0]) != 0x02) {
            log(String.format(Locale.US, "v25 0x002A canary guard: refusing because byte0 is 0x%02X, not the observed stable 0x02 profile/config selector.", ByteUtil.u8(value[0])));
            return false;
        }
        int bodyWord = ByteUtil.u16le(value, 2);
        if (bodyWord != 96) {
            log("v25 0x002A canary guard: refusing because the word at [2..3] is " + bodyWord + ", not the observed stable 96.");
            return false;
        }
        float finalFloat = ByteUtil.f32le(value, value.length - 4);
        if (Float.isNaN(finalFloat) || Float.isInfinite(finalFloat)) {
            log("v25 0x002A canary guard: refusing because the final float is not finite: " + finalFloat);
            return false;
        }
        if (finalFloat < 0.0f || finalFloat > 1.0f) {
            log("v25 0x002A canary guard: refusing because the final float is outside the conservative [0,1] range: " + finalFloat);
            return false;
        }
        return true;
    }

    private byte[] deriveOneUlpCanary2a(byte[] original) {
        ...
        float canary;
        if (current < 1.0f) {
            canary = Math.nextUp(current);
            if (canary > 1.0f || Float.isInfinite(canary) || Float.isNaN(canary)) canary = Math.nextAfter(current, 0.0);
        } else {
            canary = Math.nextAfter(current, 0.0);
        }
        if (Float.isNaN(canary) || Float.isInfinite(canary) || canary < 0.0f || canary > 1.0f || canary == current) {
            log("v25 0x002A canary guard: derived canary is not safe/distinct. current=" + current + ", canary=" + canary);
            return null;
        }
