...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v27 Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Tests whether AACP 0x0054 is semantically writable: capture the current 0x0053 vector, send a full-payload 0x0054 canary that changes only the final float32-le by one Q8 step, then immediately restore the original 0x0053 payload with 0x0054.");
        ...
        startButton.setText("Run v27 probe");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v27 probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV27Probe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v27 probe log", logBuffer.toString()));
