...
final class AacpV30HearingGateProbe {
    ...
    AacpV30HearingGateProbe(BluetoothDevice device, LogSink log) {
        this.device = device;
        this.log = log;
    }

    void run() {
        log("=== AirPods AACP v30 hearing-aid-gated settings probe started ===");
        ...
        log("--- v30 pivot: enable the LibrePods hearing-aid toggle before touching the settings payload ---");
        ...
        log("v30 therefore tests two missing preconditions from earlier probes: (1) send HEARING_AID ON first, and (2) use the LibrePods-style byte[2]=0x64 write shape for ATT 0x002A.");
        log("The only semantic canary is still tiny and reversible: one Q8 step on the final float32-le value, then a restore using the original value with the same write-shape marker.");
        log("v30 sends no AACP 0x0052, 0x0053, or 0x0055 candidates. They are observed only as responses.");
        ...
        for (int i = 1; i <= attempts; i++) {
            ...
            if (completed) break;
            if (i < attempts) {
                log("v30 hearing gate: attempt " + i + " did not reach the guarded test. Sleeping briefly, then retrying once.");
                sleep(1300);
            }
        }
        if (!completed) {
            log("v30 final result: no guarded current-session 0x002A context / 0x0053 context was available, so the gated test was not sent.");
        }
        log("=== Probe finished ===");
    }
    ...
    private boolean runHearingGateAttempt(int attempt, int totalAttempts) {
        ...
        String prefix = "v30 attempt " + attempt + "/" + totalAttempts;
        try {
            ...
            log(prefix + ": continuing with v20-style post-CCCD ATT reads before the hearing-aid toggle.");
            ...
            if (context53 != null) {
                ...
                log(prefix + ": no 0x0053 context captured. That is OK for the ATT settings path; v30 will skip the optional 0x0054 check.");
            }
            ...
            log("--- " + prefix + " BLOCK B: ATT 0x002A LibrePods-style byte[2]=0x64 Q8 canary after HEARING_AID ON ---");
            boolean canaryOk = attWriteRequest(att, HANDLE_HEARING_CONFIG, canary2a, prefix + " LibrePods-style 0x002A Q8 canary write");
            ...
            if (postCanary2a != null) {
                log("  post-canary final float=" + finalFloat(postCanary2a)
                        + ", equals original read=" + ByteUtil.equalsBytes(postCanary2a, source2a)
                        + ", final-float equals canary=" + nearlyEqual(finalFloat(postCanary2a), finalFloat(canary2a)));
            }
            if (postRestore2a != null) {
                log("  post-restore final float=" + finalFloat(postRestore2a)
                        + ", equals source read=" + ByteUtil.equalsBytes(postRestore2a, source2a)
                        + ", final-float equals source=" + nearlyEqual(finalFloat(postRestore2a), finalFloat(source2a)));
            }
    ...
    private boolean validate2aForLibrePodsWrite(byte[] v) {
        ...
        log("v30 selected ATT 0x002A settings validation: len=" + v.length + ", nonZero=" + ByteUtil.nonZero(v));
        decode2a("v30 selected ATT 0x002A settings source decode", v);
        if (v.length != 104) {
            log("v30 ATT 0x002A guard: refusing because length is " + v.length + ", expected observed 104.");
            return false;
        }
        if (ByteUtil.u8(v[0]) != 0x02) {
            log("v30 ATT 0x002A guard: refusing because byte[0] is not 0x02.");
            return false;
        }
        int declared = ByteUtil.u16le(v, 2);
        if (declared != 0x0060 && declared != 0x0064) {
            log("v30 ATT 0x002A guard: refusing because byte[2..3] is neither read-shape 0x0060 nor write-shape 0x0064. Observed=" + declared);
            return false;
        }
        float f = finalFloat(v);
        if (!Float.isFinite(f) || f < 0.0f || f > 1.0f) {
            log("v30 ATT 0x002A guard: refusing because final float " + f + " is not finite in [0,1].");
            return false;
        }
        if (Float.isNaN(chooseDelta(f, Q8_STEP))) {
            log("v30 ATT 0x002A guard: refusing because +/-Q8 would leave [0,1] for final float " + f + ".");
            return false;
        }
        return true;
    }
    ...
    private byte[] makeLibrePodsStyle2aWritePayload(byte[] source, boolean canary, String label) {
        ...
        float oldFinal = ByteUtil.f32le(source, off);
        float target = oldFinal;
        if (canary) {
            float delta = chooseDelta(oldFinal, Q8_STEP);
            target = oldFinal + delta;
        }
        ...
        log(label + String.format(Locale.US,
                ": LibrePods-style write marker byte[2..3] %d -> %d, final float %.8f (%s) -> %.8f (%s), canary=%s",
                oldDeclared, ByteUtil.u16le(out, 2), oldFinal, ByteUtil.hex(before), target, ByteUtil.hex(after), canary));
    ...
    private void log2aSemanticComparisons(String label, byte[] sourceRead, byte[] writePayload, byte[] readback) {
        ...
        log(label + String.format(Locale.US,
                ": final floats source=%.8f, writePayload=%.8f, readback=%.8f; readback==sourceFinal=%s, readback==writeFinal=%s",
                finalFloat(sourceRead), finalFloat(writePayload), finalFloat(readback),
                nearlyEqual(finalFloat(sourceRead), finalFloat(readback)),
                nearlyEqual(finalFloat(writePayload), finalFloat(readback))));
    }
    ...
    private void interpretHearingGate(String prefix,
        ...
        boolean canaryPersisted = postCanary2a != null && nearlyEqual(finalFloat(postCanary2a), finalFloat(canary2a));
        boolean canaryStayedOriginal = postCanary2a != null && nearlyEqual(finalFloat(postCanary2a), finalFloat(source2a));
        boolean restoreOriginal = postRestore2a != null && nearlyEqual(finalFloat(postRestore2a), finalFloat(source2a));
        log(prefix + " interpretation inputs: enableSawControl52=" + enableSummary.saw52()
                ...
                + ", canaryPersistedByFinalFloat=" + canaryPersisted
                + ", canaryStayedOriginalByFinalFloat=" + canaryStayedOriginal
                + ", restoreReturnedOriginalByFinalFloat=" + restoreOriginal + ".");
        if (canaryPersisted) {
            log(prefix + " interpretation: strong win. With HEARING_AID ON and the LibrePods byte[2]=0x64 write shape, ATT 0x002A reflected the canary. That means earlier failures were probably due to missing enable/write-shape preconditions.");
    ...
    private boolean validate53ForOptional54(byte[] payload) {
        ...
        log("v30 optional 0x0053 validation: len=" + payload.length + ", declared/body-ish=" + declared + ", float summary=" + ByteUtil.floatSummary(payload));
