...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v30 Hearing Gate Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Hearing-gated settings probe: turns HEARING_AID on with AACP control 0x0009/0x2C, then writes a reversible LibrePods-style ATT 0x002A payload using byte[2]=0x64 and a one-Q8-step final-float canary.");
        ...
        startButton.setText("Run v30 hearing-gate");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v30 hearing-gate probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV30HearingGateProbe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v30 hearing-gate probe log", logBuffer.toString()));
