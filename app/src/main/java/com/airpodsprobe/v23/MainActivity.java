...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v23 Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Tests the next safe 0x0054 wire-shape: capture the current-session 0x0053 vector on the winning 0x0022 path, strip its leading 2-byte length word, then send exactly one body-only no-op 0x0054 candidate and log direct vs refresh responses separately.");
        ...
        startButton.setText("Run v23 probe");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v23 probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV23Probe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v23 probe log", logBuffer.toString()));
