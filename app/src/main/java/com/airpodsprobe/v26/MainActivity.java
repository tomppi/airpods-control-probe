...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v26 Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Tests whether ATT 0x002A is semantically writable: verify the current 0x002A value is stable, write a Q8-step canary to only the final float32-le value, read it back, then immediately restore the exact original bytes.");
        ...
        startButton.setText("Run v26 probe");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v26 probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV26Probe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v26 probe log", logBuffer.toString()));
