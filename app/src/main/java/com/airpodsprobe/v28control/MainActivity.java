...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v28 Control Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Pure control-map probe: no canary and no ATT 0x002A writes. Captures current 0x0053, then isolates baseline, refresh-only, original 0x0053 via 0x0054, and post-0x0054 refresh blocks to determine whether 0x0052 is tied to 0x0054.");
        ...
        startButton.setText("Run v28 control");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v28 control probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV28ControlProbe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v28 control probe log", logBuffer.toString()));
