...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v21 Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Single-candidate test: uses the v20 winning trigger path, captures the current AACP 0x0053 vector, sends exactly one AACP 0x0054 no-op echo, then watches for 0x0055/0x0053/0x0052 and ATT 0x002A changes.");
        ...
        startButton.setText("Run v21 probe");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v21 probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV21Probe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v21 probe log", logBuffer.toString()));
