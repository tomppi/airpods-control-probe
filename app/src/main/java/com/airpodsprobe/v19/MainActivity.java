...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v19 Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Sweeps AACP 0x004D feature flags, captures 0x0053/0x0055, then tests no-op 0x0054. Requires paired AirPods and the existing LibrePods Xposed module in com.android.bluetooth.");
        ...
        startButton.setText("Run v19 probe");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v19 probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV19Probe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v19 probe log", logBuffer.toString()));
