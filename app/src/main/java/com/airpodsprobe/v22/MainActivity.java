...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v22 Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Faithful v20 replay: continues past the v21 abort point with post-CCCD ATT reads and a deeper AACP drain, then sends exactly one AACP 0x0054 no-op echo only if a current-session 0x0053 vector is captured.");
        ...
        startButton.setText("Run v22 probe");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v22 probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV22Probe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v22 probe log", logBuffer.toString()));
