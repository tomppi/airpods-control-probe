...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v24 Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Tests the next safe setter-path hypothesis: use the same winning 0x0022 path, verify ATT 0x002A is stable across reads, then send exactly one ATT Write Request that writes the exact current 0x002A bytes back unchanged and log ATT/AACP responses.");
        ...
        startButton.setText("Run v24 probe");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v24 probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV24Probe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v24 probe log", logBuffer.toString()));
