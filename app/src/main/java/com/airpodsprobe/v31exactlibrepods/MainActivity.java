...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("AirPods AACP v31 Exact LibrePods Hearing Probe");
        title.setTextSize(20);
        title.setGravity(Gravity.START);
        root.addView(title, new LinearLayout.LayoutParams(-1, -2));

        TextView desc = new TextView(this);
        desc.setText("Exact LibrePods hearing settings probe: enables ATT 0x002A notifications via CCCD 0x002B, turns HEARING_AID on with AACP 0x0009/0x2C, then writes a reversible byte[2]=0x64 payload changing left_eq[0] by +1.0 dBHL.");
        ...
        startButton.setText("Run v31 exact-librepods");
    ...
    private void startProbe() {
        ...
        appendLog("Starting v31 exact-librepods probe for " + mac + "...");
        executor.execute(() -> {
            try {
                new AacpV31ExactLibrePodsHearingProbe(device, this::appendLog).run();
            } catch (Throwable t) {
    ...
    private void copyLog() {
        ...
        if (cm != null) {
            cm.setPrimaryClip(ClipData.newPlainText("AACP v31 exact-librepods probe log", logBuffer.toString()));
